	...
	function set_amount(val:Float) {
		...
		@:glFragmentSource('
			...
			void main() {
				...
				vec4 a = posterize(flixel_texture2D(bitmap, quantize(uv, 64.0 * t) + pixel * (postColor.rb - vec2(0.5)) * 100.0), 5.0).rbga;
				vec4 b = posterize(flixel_texture2D(bitmap, quantize(uv, 32.0 - t) + pixel * (postColor.rg - vec2(0.5)) * 1000.0), 4.0).gbra;
				vec4 c = posterize(flixel_texture2D(bitmap, quantize(uv, 16.0 + t) + pixel * (postColor.rg - vec2(0.5)) * 20.0), 16.0).bgra;
				gl_FragColor = mix(flixel_texture2D(bitmap, uv + amount * (quantize((a * t - b + c - (t + t / 2.0) / 10.0).rg, 16.0) - vec2(0.5)) * pixel * 100.0), (a + b + c) / 3.0, (0.5 - (dot(color, postColor) - 1.5)) * amount);
			}
		')
	#else
		@:glFragmentSource('
			...
			void main() {
				...
				gl_FragColor = mix(flixel_texture2D(bitmap, uv + amount * (quantize((a * t - b + c - (t + t / 2.0) / 10.0).rg, 16.0) - vec2(.5)) * pixel * 100.0), (a + b + c) / 3.0, (0.5 - (dot(color, postColor) - 1.5)) * amount);
			}
		')
	#end
	...
	function set_amount(val:Float) {
		...
		@:glFragmentSource('
			...
			vec3 edge(sampler2D sampler, vec2 uv, float sampleSize) {
				...
				return (mix(downsample(sampler, uv - vec2(dx, 0.0), sampleSize), downsample(sampler, uv + vec2(dx, 0.0), sampleSize), mod(uv.x, dx) / dx) + mix(downsample(sampler, uv - vec2(0.0, dy), sampleSize), downsample(sampler, uv + vec2(0.0, dy), sampleSize), mod(uv.y, dy) / dy)).rgb / 2.0 - flixel_texture2D(sampler, uv).rgb;
			}
